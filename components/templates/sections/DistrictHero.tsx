...
export const DistrictHero = ({ districts = [], province }: DistrictHeroProps) => {
  ...
  const safeDistricts = useMemo(() => (districts.length > 0 ? districts : ["เมือง"]), [districts]);
  ...
  if (!mounted) {
    return (
      <div className="border-y border-transparent bg-[var(--brand-primary)]/5 py-4 opacity-0">
        <div className="container mx-auto h-6 px-4" />
      </div>
    );
  }
  ...
  return (
    <div className="animate-in fade-in border-y border-[var(--foreground)]/5 bg-[var(--brand-primary)]/10 py-4 transition-opacity duration-700 ease-in">
      <div className="container mx-auto px-4">
        <div className="flex flex-col items-center justify-center gap-2 md:flex-row md:gap-4">
          ...
          <div className="flex items-center gap-2 text-sm font-bold text-[var(--foreground)] italic">
            ...
            <div className="relative h-6 w-[140px] overflow-hidden text-center md:text-left">
              ...
              <span
                ...
                className="animate-in slide-in-from-bottom-1 fade-in absolute inset-0 block text-[var(--brand-primary)] duration-500"
              >
                อ. {currentDistrict}
              </span>
            </div>
