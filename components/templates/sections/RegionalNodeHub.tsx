/**
 * [SYSTEM COMPONENT]: REGIONAL_NODE_HUB v18.0.1 (SEO_ACCELERATOR)
 * [STRATEGY]: Programmatic Internal Linking | Geographic Authority | Zero-CLS
 * [MAINTAINER]: AEMZA MACKS (Lead Architect)
 */

import React, { memo } from "react";
import Link from "next/link";
import type { UniversalTemplateProps } from "@/types/template-props";
import IconRenderer from "@/components/ui/IconRenderer";

const RegionalNodeHub = ({ data }: { data: UniversalTemplateProps }) => {
  const { servingAreas, province, masterServiceUrl, title } = data;

  // [LOGIC]: If we have servingAreas, we are likely on a Main Service Page
  if (servingAreas && servingAreas.length > 0) {
    return (
        ...
        <div className="container mx-auto px-4">
          <header className="mb-16 space-y-6 text-center">
            <div className="border-brand-primary/20 bg-brand-primary/5 text-brand-primary mx-auto inline-flex items-center gap-3 rounded-full border px-6 py-2 font-mono text-[10px] font-black tracking-[0.4em] uppercase backdrop-blur-md">
              ...
              <span>Regional_Service_Network</span>
            </div>
            <h2 className="text-text-primary text-4xl font-black tracking-tighter uppercase italic md:text-6xl">
              Serving Across <span className="text-brand-primary">Thailand.</span>
            </h2>
  ...
  // [LOGIC]: If we have a masterServiceUrl and we are on an Area Page
  if (province && masterServiceUrl) {
    return (
      <section className="bg-surface-main py-20">
        <div className="container mx-auto px-4">
