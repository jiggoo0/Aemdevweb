...
import { BlogPostProps } from "@/types" // ✅ ใช้งาน Interface ที่อัปเดตแล้วจาก types/index.ts

/**
 * 📝 BlogCard: Luminous Edition
 * การ์ดบทความที่ฉีกความหมองด้วยระบบแสง Aurora และ Weightless Design
 */
export function BlogCard({
  slug,
  title,
  description, // ✅ รับได้แล้วหลังจากอัปเดต types/index.ts
  coverImage,
  date,
  category,
  readingTime = "5 นาที",
      ...
      <article className="glass-card hover:border-aurora-cyan/30 hover:shadow-luminous relative flex h-full flex-col overflow-hidden rounded-[2.5rem] border border-white/5 bg-white/[0.02] backdrop-blur-xl transition-all duration-700 hover:-translate-y-3">
        ...
        <div className="flex flex-1 flex-col p-8 md:p-10">
          ...
          <div className="mb-6 flex items-center gap-5 text-[10px] font-black tracking-[0.25em] text-slate-500 uppercase">
            <div className="group-hover:text-aurora-emerald flex items-center gap-2 transition-colors">
            ...
            <div className="group-hover:text-aurora-cyan flex items-center gap-2 transition-colors">
              <Clock className="text-aurora-cyan/60 h-3.5 w-3.5" />
              <span>{readingTime}</span>
            </div>
          </div>

          {/* Title: Strong Luminous Title */}
          <h3 className="font-prompt group-hover:text-aurora-cyan mb-4 line-clamp-2 text-2xl leading-[1.2] font-black tracking-tighter text-white uppercase italic transition-colors">
            {title}
          </h3>

          {/* Description: Friendly Anuphan Font */}
          <p className="font-anuphan mb-8 line-clamp-3 flex-1 text-base leading-relaxed font-medium text-slate-400 opacity-80 transition-opacity group-hover:opacity-100">
            {description}
          </p>

          {/* --- 3. Action Link: Luminous Footer --- */}
          <div className="mt-auto flex items-center justify-between border-t border-white/5 pt-8">
            <span className="flex items-center gap-2 text-[10px] font-black tracking-[0.3em] text-slate-500 uppercase transition-all group-hover:text-white">
              Read Article{" "}
