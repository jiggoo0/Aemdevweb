/**
 * [ROUTE PAGE]: CASE_STUDIES_HUB_SYSTEM v18.0.0 (LCP_OPTIMIZED)
 */
...
// [INSIGHT]: บังคับ Static Generation แต่ให้ Revalidate ทุก 1 ชม. กันเหนียว
export const revalidate = 3600;

export const metadata: Metadata = constructMetadata({
  title: "Case Studies | ผลงานวิศวกรรมเว็บระดับสูง",
  description:
    "รวบรวมโปรเจกต์ที่ผ่านการ Optimize ขั้นสูงสุด ตั้งแต่ระดับโครงสร้างจนถึง Conversion Rate",
  path: "/case-studies",
...
export default async function CaseStudiesPage() {
  ...
  const fullSchema = generateSchemaGraph([
    ...
    // [SEO_DEEP_INSIGHT]: เพิ่ม CollectionPage schema เพื่อบอก Google ว่านี่คือศูนย์รวมข้อมูลสำคัญ
    {
      "@type": "CollectionPage",
      name: "AEMDEVWEB Success Archive",
      description: "Professional Web Development Case Studies",
    },
  ]);
        ...
        <section>
          <header className="mb-24 max-w-4xl space-y-10 md:mb-32">
            <div className="border-brand-primary/20 bg-brand-primary/5 text-brand-primary inline-flex items-center gap-4 rounded-full border px-5 py-2 font-mono text-[10px] font-black tracking-[0.4em] uppercase backdrop-blur-md">
              ...
              <span>Project_Success_Ref.v18.0.0</span>
            </div>
            ...
            <div className="border-brand-primary border-l-4 pl-8 md:pl-14">
              <p className="text-text-secondary max-w-2xl text-xl leading-relaxed font-medium italic opacity-90 md:text-2xl">
                เราเปลี่ยน "ความต้องการ" ให้เป็น "ผลลัพธ์" ผ่านกระบวนการ Technical Audit
                ที่ไม่มีในตำราทั่วไปครับ
              </p>
            </div>
          </header>
          ...
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-16">
            {cases.map((item, index) => (
              <CaseStudyCard
                ...
                data={item}
                index={index}
                // [FIXED]: ส่ง priority ให้ 2 ใบแรกเพื่อเร่งคะแนน LCP
                priority={index < 2}
              />
            ))}
          </div>
