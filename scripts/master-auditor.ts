...
async function auditSemantic() {
  ...
  const thaiExpertKeywords = [
    'ผู้เชี่ยวชาญ', 'วางระบบ', 'เฉพาะทาง', 'ยกระดับ', 'สถาปัตยกรรม',
    'มาตรฐานสากล', 'ความน่าเชื่อถือ', 'ปรับจูน', 'ประสิทธิภาพ', 'ปลอดภัย'
  ];
  ...
  for (const file of blogFiles) {
    ...
    if (hasThaiExpertise && hasStructure) {
      ...
      result.findings.push(`⚠️ [${file}]: เนื้อหาอาจขาด 'สัญญาณความเชี่ยวชาญ' (เช่น คำว่า วางระบบ, ผู้เชี่ยวชาญ)`);
    }
  }

  result.score = Math.round((expertContentCount / blogFiles.length) * 100);
  if (result.score === 100) {
    result.findings.push("✅ ทุกบทความมีระดับเนื้อหาที่เป็นมืออาชีพและรองรับความต้องการของลูกค้าไทย");
  }
