...
function generateAiPrompt(province: string, region: string, slug: string): string {
  ...
  const technical =
    "8k UHD, luxury commercial style, clean lines, sunset lighting, cinematic, no people, sharp focus, professional color grading";

  let context = "";
  if (slug === "bangkok")
    context = "futuristic Bangkok CBD skyline with luxury skyscrapers, dusk atmosphere";
  else if (["phuket", "krabi", "phang-nga"].includes(slug))
    context = `luxury beach resort and emerald sea in ${province}, Andaman coast`;
  else if (["chiang-mai", "chiang-rai", "nan"].includes(slug))
    context = `modern Lanna architecture and mountain resort in ${province}`;
...
async function getProvinceNodes(): Promise<ProvinceNode[]> {
  ...
  const files = fs
    ...
    .filter((f) => f.endsWith(".ts") && f !== "index.ts" && f !== "node-factory.ts");
...
async function main() {
  console.log(
    `🛠️  Initializing AI Asset Architect v3.6.2 [${TEST_ONLY ? "TEST MODE" : "FULL MODE"}]...`,
  );
  ...
  for (const node of nodes) {
    ...
    if (buffer) {
      ...
      if (success) {
        console.log(
          `   ✅ [CREATED]: ${filename} (${(fs.statSync(dest).size / 1024).toFixed(1)} KB)`,
        );
        manifest[node.slug] = {
